...
class XYZProcessor(ProcessorMixin):
    ...
    def _tokenize_with_dropout(
        ...
        **kwargs,
        ...
        with attr(self.tokenizer if backend_tokenizer is None else backend_tokenizer.model, dropout=bpe_dropout):
    ...
    def apply_crop(self, examples: dict, max_length: int | None = None) -> dict:
        for idx, text in enumerate(examples[self.text_column]):
            ...
            if max_length and max_length < n:
                ...
                if "residue_idx" in examples:
                    examples["residue_idx"][idx] = feat_crop(examples["residue_idx"][idx])
        return examples
