...
class XYZForCausalLM(LlamaForCausalLM):
    def __init__(self, config: XYZConfig):
        ...
        if config.has_cle_lm_head:
            self.cle_lm_head = nn.Linear(
                config.char_hidden_size, config.cle_vocab_size, bias=False
            )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        model_inputs.update(
            processor.to_characterization(
                processor.to_example(input_ids), model_inputs, generate=True
            )
        )
        return model_inputs
...
class XYZProcessor(ProcessorMixin):
        ...
        if self.features is not None:
            ...
            if "distogram_labels" in self.features:
                if "distogram_labels" in tokenized:
                    ...
                    tokenized["distogram_labels"] = torch.cat(
                        (pseudo_beta, pseudo_beta_mask[..., None]), dim=-1
                    )
    ...
    def to_characterization(
        ...
        char_offset_mapping = characterized["char_offset_mapping"].where(
            characterized["char_attention_mask"][..., None] > 0,
            characterized["char_offset_mapping"].max() + 1
        )
        offset_mapping = tokenized["offset_mapping"].where(
            tokenized["attention_mask"][..., None] > 0,
            tokenized["offset_mapping"].max() + 1
        )
        ...
        characterized["repr_char_idx"] = characterized["repr_char_idx"].where(
            tokenized["attention_mask"] > 0, 0
        )
